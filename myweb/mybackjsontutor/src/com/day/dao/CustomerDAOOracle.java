...
public class CustomerDAOOracle implements CustomerDAO{
	...
	public Customer selectById(String id) throws FindException {
		...
		try {
			...
			throw new FindException("?ïÑ?ù¥?îîÍ∞? Ï°¥Ïû¨?ïòÏß? ?ïä?äµ?ãà?ã§");
		}catch(SQLException e) {
			throw new FindException(e.getMessage());
		}finally {
	...
	public void update(Customer c) throws ModifyException {
		...
		boolean flag = false; //Î≥?Í≤ΩÌï† Í∞íÏù¥ ?ûà?äî Í≤ΩÏö∞ true
		...
		if(!flag) {
			throw new ModifyException("?àò?†ï?ï† ?Ç¥?ö©?ù¥ ?óÜ?äµ?ãà?ã§");
		}
