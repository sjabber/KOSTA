...
public class OrderInfo {
	...
	@ManyToOne
	//@JoinColumn은 자식엔티티 클래스에서만 사용할 수 있다.
	//name속성값은 자식엔티티 테이블의 컬럼명을 적어준다.
	@JoinColumn(name = "order_id")
//	@Transient //테이블 컬럼과 관련없다고 선언할 때 사용한다.
	...
	@OneToMany(
			...
			, 
			//JPA는 연관관계가 있는 엔티티를 조회할 때 default로 지연로딩을 합니다. 여러 엔티티들과 종속적인 관계를 맺고 있다면 조인이 필요, 조인이 복잡할 수록 성능저하!, 정보가 필요하기 전까지 최대한 테이블에 접근하지 않는 방식을 사용합니다.
			//즉시 로딩은 조인은 이용해서 정보를 처리합니다.
			//즉시로딩을 안한다면 @Transactional로 처리할 수 도 있습니다.
			fetch = FetchType.EAGER
			//,mappedBy = "order_info" //부모쪽에만 선언한다.
			)
